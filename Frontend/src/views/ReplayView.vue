  ...
  <Suspense>
    ...
    <div>
      <div class="text-center mb-4">
        <h2>Rediffusion de la partie</h2>
        <p style="font-size: 1.5rem; color: #fff">
          Étape {{ currentStepIndex + 1 }} sur {{ replayData.actions.length }}
        </p>
        <p style="color: #fff">
          Joueur : {{ replayData.ownerUsername }} / Couleur :
          {{ replayData.ownerColor === Color.BLACK ? 'Noirs' : 'Blancs' }}
        </p>
      </div>
          ...
          <div class="card p-4">
            ...
            <div class="flex flex-column gap-3">
              ...
              <Button
                ...
                @click="nextStep"
                :disabled="currentStepIndex === replayData.actions.length - 1"
                label="Coup suivant"
                class="p-button-outlined w-full"
              />
...
onMounted(async () => {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Erreur',
      detail: 'Impossible de charger le replay',
      life: 5000,
    });
    router.push('/');
  }
  ...
  if (currentStepIndex.value < replayData.value.actions.length - 1) {
    ...
    if (destinationPiece && step.pieceKilled) {
      const capturedPieces =
        step.pieceKilled.color === Color.BLACK ? capturedBlackPieces : capturedWhitePieces;
      ...
      const killedPieces =
        step.pieceKilled.color === Color.BLACK ? blackKilledPieces : whiteKilledPieces;
    ...
    if (movedPiece) {
      ...
      if (step.pieceKilled) {
        ...
        const capturedIndex = capturedPieces.value.findIndex(
          (p) =>
            p.pieceType === step.pieceKilled?.pieceType && p.i === step.toI && p.j === step.toJ,
        );

        if (capturedIndex !== -1) {
          const removedPiece = capturedPieces.value.splice(capturedIndex, 1)[0];
          killedPieces.value.splice(
            killedPieces.value.findIndex((p) => p === removedPiece.pieceType),
            1,
          );
