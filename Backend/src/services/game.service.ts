...
export class GameService {
  ...
  async getReplay(gameId: number): Promise<ChessReplay | undefined> {
    const game = await Game.findByPk(gameId, {
      include: [
        { model: GameAction, as: "gameAction", order: [["id", "ASC"]] },
        { model: User, as: "owner" },
      ],
    });
    ...
    gameActions.forEach((element, index) => {
      ...
      if (element.piece) {
        ...
        const index = listKilledPiece.findIndex(
          (e) => e.color == color && e.pieceType == piece
        );
        if (index != -1) {
    ...
    let replay: ChessReplay = {
      actions: actionList,
      ownerColor: game.owner_color,
  ...
  async movePiece(
    ...
    const resultMovePiece = movePiece(
      game,
      movePieceBody.i,
      movePieceBody.j,
      movePieceBody.toI,
    ...
    const resultCheckTowerUpgrade = checkTowerUpgrade(
      game,
      movePieceBody.toI,
      movePieceBody.toJ
    );
