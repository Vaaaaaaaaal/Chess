...
export class King extends Piece {
  possibleMove(listCase: Case[][]): ChessLocation[] {
    ...
    for (const [di, dj] of directions) {
      ...
      if (
        newI >= 0 &&
        newI < listCase.length &&
        newJ >= 0 &&
        newJ < listCase[0].length
      ) {
  ...
  public static checkKingStatus(
    listCase: Case[][],
    king: King
  ): { king: King; status: string } {
  ...
  public static canKingMove(listCase: Case[][], king: King): boolean {
    ...
    for (let { di, dj } of directions) {
      ...
      if (
        isInBounds(newI, newJ) &&
        !this.isOccupiedByOwnPiece(listCase, king, newI, newJ)
      ) {
        let isSafe = true;
        ...
        for (let row of listCase) {
          for (let c of row) {
            if (
              c.piece &&
              c.piece.color !== king.color &&
              c.piece.move(newI, newJ, listCase)
            ) {
              isSafe = false;
              break;
            }
  ...
  public static isOccupiedByOwnPiece(
    listCase: Case[][],
    king: King,
    i: number,
    j: number
